...
const Audio = ({ children, src, ...props }) => {
  ...
  return (
    <AudioContext.Provider value={audioState}>
      <div className="audio-component">{children}</div>
...
const AudioControls: React.FC = () => {
  const { isPlaying, currentTrackId, playTrack, pauseTrack } = useAudioStore();
...
const AudioApp: React.FC = () => {
  ...
  return (
    <div>
      <button onClick={() => setShowPlayer(true)}>Load Audio Player</button>
...
const VirtualizedAudioList: React.FC<AudioListProps> = ({
  ...
  const renderAudioItem = useCallback(
    ({ index, style }) => {
      ...
      return (
        <div style={style}>
          <AudioListItem key={track.id} track={track} isVirtualized={true} />
        </div>
      );
    },
    [tracks]
...
const AudioEqualizer = memo<{
  ...
  (prevProps, nextProps) => {
    ...
    for (let i = 0; i < prevProps.frequencies.length; i++) {
      if (
        Math.abs(prevProps.frequencies[i] - nextProps.frequencies[i]) > 0.001
      ) {
        return false;
      }
    }
...
describe("AudioPlayer Integration", () => {
  ...
  it("should seek when progress bar is clicked", async () => {
    ...
    act(() => {
      const loadedHandler = mockAudio.addEventListener.mock.calls.find(
...
describe("AudioPlayer Accessibility", () => {
  it("should have proper ARIA labels", () => {
    ...
    expect(
      screen.getByRole("button", { name: /play test audio/i })
    ).toBeInTheDocument();
    expect(
