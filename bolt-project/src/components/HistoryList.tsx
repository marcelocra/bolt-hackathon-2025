...
import { Play, Pause, Trash2, Calendar, Clock, Loader2, ChevronDown, ChevronUp, FileText, MoreVertical } from 'lucide-react';
...
export const HistoryList: React.FC<HistoryListProps> = ({ refreshTrigger }) => {
  ...
  const updateProgress = () => {
    if (playingAudio) {
      const progress = (playingAudio.audio.currentTime / playingAudio.audio.duration) * 100;
  ...
  const truncateText = (text: string, maxLines: number = 2): { truncated: string; needsExpansion: boolean } => {
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-700/50 overflow-hidden">
        ...
        <div className="divide-y divide-slate-700/30">
          {entries.map((entry) => {
            ...
            const transcriptData = entry.transcription ? truncateText(entry.transcription) : null;
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <button
                        ...
                        disabled={!entry.original_audio_url && !entry.processed_audio_url}
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      {isMenuOpen && (
                        <div className="absolute right-0 top-12 bg-slate-800 border border-slate-700/50 rounded-lg shadow-2xl z-10 min-w-48">
                          <div className="py-2">
                            <button
                              onClick={() => entry.transcription ? toggleTranscript(entry.id) : generateTranscript(entry.id)}
                            ...
                            >
                              {transcribingId === entry.id ? (
                                ...
                                <>
                                  ...
                                  <span>
                                    {entry.transcription ? 'View Transcript' : 'Generate Transcript'}
                                  </span>
                                </>
                              )}
                            </button>
                  ...
                  {isPlaying && playingAudio && (
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-slate-400">
                        <span>{formatProgress(playingAudio.audio.currentTime)}</span>
                      ...
                      <div className="text-center space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-sm">
                            This will permanently delete "{entry.title}" and all associated audio files. This action cannot be undone.
                          </p>
                        </div>
